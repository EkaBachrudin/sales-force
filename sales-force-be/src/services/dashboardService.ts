 ...
 */
export const getDashboardOverview = async (userId: string): Promise<DashboardOverviewMetrics> => {
  ...
  const totalLeadsResult = await pool.query(
    `SELECT COUNT(*) as count FROM leads WHERE assigned_to = $1 AND status != 'cancelled'`,
    [userId]
  );
  ...
  const surveyedResult = await pool.query(
    `SELECT COUNT(*) as count FROM leads WHERE assigned_to = $1 AND status = 'surveyed'`,
    [userId]
  );
