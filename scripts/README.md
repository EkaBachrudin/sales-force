...
| Script | Description |
|--------|-------------|
| [backup-db.sh](backup-db.sh) | Automated daily backup script |
| [restore-db.sh](restore-db.sh) | Restore database from backup |
| [sync-backup.sh](sync-backup.sh) | Sync backups to cloud storage (S3, GDrive, Dropbox, SFTP) |
| [list-backups.sh](list-backups.sh) | List available backups |
| [auto-backup.sh](auto-backup.sh) | Wrapper script: backup + sync in one step |
| [setup-backup-automation.sh](setup-backup-automation.sh) | One-click setup automation (cron/systemd) |
...
## ☁️ Cloud Storage Setup (Rclone)

For `sync-backup.sh` to work with cloud storage, install and configure rclone:
