  ...
  try {
    ...
    const updateQuery = `
      UPDATE leads SET
        ...
        last_followed_up_at = COALESCE($11, last_followed_up_at),
        next_follow_up_at = COALESCE($12, next_follow_up_at),
        property_price = COALESCE($13, property_price),
        down_payment_percentage = COALESCE($14, down_payment_percentage),
        interest_rate = COALESCE($15, interest_rate),
        loan_term_years = COALESCE($16, loan_term_years),
        estimated_monthly_payment = COALESCE($17, estimated_monthly_payment),
        updated_at = NOW()
      WHERE id = $1
      RETURNING *
    `;
